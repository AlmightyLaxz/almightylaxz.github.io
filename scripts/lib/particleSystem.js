...
{	
	...
	for(var i=0; i<this.particles.length; i++ )
	{
		...
		context.drawImage(this.texture, origin.x * scale.x - worldOffsetX, origin.y * scale.y, p.size.x, p.size.y);
		context.restore();		
	}
}
...
var SingleEmitter = function(imageFilename, positionX, positionY) 
{
	this.particles = [];
	this.elapsedEmittionTime = 0;
...
SingleEmitter.prototype.update = function(dt) 
{	
	this.elapsedEmittionTime += dt;
	this.life -= dt;

	if(!this.fired) {this.spawnParticle(); this.fired = true;}
...
SingleEmitter.prototype.draw = function() 
{	
	var origin = new Vector2();
	origin.set(this.texture.width / 2, this.texture.height / 2);
...
SingleEmitter.prototype.spawnParticle = function()
{
	var p = new Particle();
